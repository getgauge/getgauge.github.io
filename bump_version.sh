...
then
  ...
  exit 1
   ...
   darwin86=`openssl sha1 deploy/gauge-$1-darwin.x86.zip | cut -d " " -f2`
   darwin86_64=`openssl sha1 deploy/gauge-$1-darwin.x86_64.zip | cut -d " " -f2`
   ...
   windows86=`openssl sha1 deploy/gauge-$1-windows.x86.zip | cut -d " " -f2`
   windows86_64=`openssl sha1 deploy/gauge-$1-windows.x86_64.zip | cut -d " " -f2`
